...
GLM_FUNC_QUALIFIER bool intersectRayPlane
    ...
    {
        typename genType::value_type const tmp_intersectionDistance =
                glm::dot(planeOrig - orig, planeNormal) / d;
        if (tmp_intersectionDistance >
            static_cast<typename genType::value_type>(0)) { // allow only intersections
...
vec<3, T, Q> const &dir,
        vec<3, T, Q>
const& vert0,
vec<3, T, Q> const &vert1, vec<3, T, Q>
const& vert2,
...
x = glm::dot(dist, p);
if(baryPosition.
x<static_cast<T>(0) || baryPosition.x> det
)
return false;
